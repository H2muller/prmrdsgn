...
def bowtie2_alignment(forward_primer_list,reverse_primer_list,genome):
    ...
    align = f'bowtie2 -p {cpu_count()} -x {genome}_index -X {args.a + 2*(args.l)} -f -1 {forward_primer_list} -2 {reverse_primer_list} {make_sam} -D 50 -N 1 --local --no-overlap --no-contain {quiet} 2> markers.txt'
    if args.verbose:
        print(f'Aligning with Bowtie2 – {cpu_count()} threads will be utilized')
